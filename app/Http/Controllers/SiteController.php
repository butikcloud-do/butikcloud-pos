...
class SiteController extends Controller
{
    ...
    public function features()
    {
        $pageTitle   = "Features";
        $user        = auth()->user();
        $sections    = Page::where('tempname', activeTemplate())->where('slug', 'features')->first();
        $seoContents = $sections->seo_content;
        $seoImage    = @$seoContents->image ? getImage(getFilePath('seo') . '/' . @$seoContents->image, getFileSize('seo')) : null;
        return view('Template::features', compact('pageTitle', 'user', 'sections', 'seoContents', 'seoImage'));
    }
    ...
    public function pricingPlan()
    {
        ...
        $user        = auth()->user();
        $sections    = Page::where('tempname', activeTemplate())->where('slug', 'pricing-plan')->first();
        $seoContents = $sections->seo_content;
        $seoImage    = @$seoContents->image ? getImage(getFilePath('seo') . '/' . @$seoContents->image, getFileSize('seo')) : null;
        return view('Template::pricing_plan', compact('pageTitle', 'user', 'sections', 'seoContents', 'seoImage'));
    }
    ...
    public function subscribe(Request $request)
    {
        ...
        return response()->json(['status' => 'success', 'message' => 'Thanks for connecting with us.']);
    }
}
