...
class ManageUsersController extends Controller
{
    ...
    public function subscribedUsers()
    {
        ...
        extract($this->userData("subscribed"));
    ...
    public function Staff($id)
    {
        ...
        $users = $baseQuery->where('parent_id', $id)->paginate(getPaginate());
        return view('admin.users.list', compact('pageTitle', 'users', 'widget'));
    }
    ...
    protected function userData($scope = 'query')
    {
        if ($scope != 'staff') {
            ...
            $baseQuery = User::searchable(['email', 'username', 'firstname', 'lastname'])
                ->staff()
                ->dateFilter()
                ->filter(['status'])
                ->orderBy('id', getOrderBy());
        ...
        return [
            'baseQuery' => $baseQuery,
            'widget'    => $this->widgetCounts(),
        ];
    }

    private function widgetCounts()
    {
    ...
    {
        $user      = User::with('plan', 'planPurchases.subscriptionPlan')->findOrFail($id);
