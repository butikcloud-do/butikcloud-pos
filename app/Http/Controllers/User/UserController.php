...
class UserController extends Controller
{
    ...
    public function depositHistory(Request $request)
    {
        ...
        $deposits = getParentUser()->deposits()->searchable(['trx'])->with(['gateway'])->orderBy('id', getOrderBy())->paginate(getPaginate());
    ...
    public function lowStockProduct()
    {
        ...
        if (request()->warehouse_id) {
            $warehouse = Warehouse::where('user_id', $user->id)->where('id', request()->warehouse_id)->first();
        } else {
    ...
    public function transactions()
    {
        ...
        $accountIds = Transaction::where('user_id', getParentUser()->id)
            ->distinct('payment_account_id')
            ->pluck('payment_account_id');

        $paymentTypeIds = Transaction::where('user_id', getParentUser()->id)
            ->distinct('payment_type_id')
            ->pluck('payment_type_id');

        $paymentTypes = PaymentType::whereIn('id', $paymentTypeIds)->get();

        $transactions = Transaction::where('user_id', getParentUser()->id)->searchable(['trx'])->filter(['trx_type', 'remark', 'payment_account_id', 'payment_type_id'])->orderBy('id', 'desc')->paginate(getPaginate());

        return view('Template::user.transactions', compact('pageTitle', 'transactions', 'remarks', 'paymentTypes'));
    }
    ...
    public function transactionsHistory()
    {
        ...
        $remarks   = Transaction::distinct('remark')->orderBy('remark')->get('remark');
        $transactions = Transaction::where('user_id', getParentUser()->id)->searchable(['trx'])->filter(['trx_type', 'remark'])->orderBy('id', 'desc')->paginate(getPaginate());
