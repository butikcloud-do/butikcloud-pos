...
class CategoryImportController extends Controller
{
    public function downloadSample()
    {
        ...
        // Define headers matching the exact fields from user/category/list
        $headers = [
            'Name'
        ];
        ...
        header('Content-Disposition: attachment;filename="category_import_sample.xlsx"');
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            try {
                $importedCount = 0;
                foreach ($validData as $categoryData) {
                    $this->createCategoryFromImport($categoryData, $user);
                    $importedCount++;
                }
                ...
                adminActivity("category-bulk-import", Category::class, 0, "Imported {$importedCount} categories successfully");
                return jsonResponse('import_success', 'success', $message);

            } catch (Exception $e) {
                DB::rollBack();
                $message[] = "Import failed: " . $e->getMessage();
                adminActivity("category-bulk-import", Category::class, 0, "Import failed: " . $e->getMessage());
    ...
    private function validateRow($row, $rowNumber, $user)
    {
        ...
        if (!empty($data['name'])) {
            $existingCategory = Category::where('user_id', $user->id)
                ...
                ->first();
            if ($existingCategory) {
                $errors[] = "Row {$rowNumber}: Category Name '{$data['name']}' already exists";
            }
        }
    ...
    private function createCategoryFromImport($data, $user)
    {
        ...
        $category->save();
        adminActivity("category-insert", get_class($category), $category->id);
    }
}
