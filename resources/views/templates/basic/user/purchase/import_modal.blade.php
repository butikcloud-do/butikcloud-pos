{{-- Purchase/Stock Import Modal - Matching Category Import Modal Style --}}
{{-- Hidden by default to prevent flash during page load --}}
<div id="importPurchaseModal" class="modal fade custom--modal" tabindex="-1" role="dialog" style="display: none;">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">@lang('Import Stock (Purchase)')</h5>
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <i class="las la-times"></i>
                </button>
            </div>
            <form action="{{ route('user.purchase.import.store') }}" method="POST" enctype="multipart/form-data" class="purchase-import-form">
                @csrf
                <div class="modal-body">
                    {{-- Instructions Section --}}
                    <div class="alert alert-info mb-4">
                        <h6 class="alert-heading mb-2">
                            <i class="las la-info-circle"></i> @lang('Bulk Stock Import Instructions')
                        </h6>
                        <ul class="mb-0 ps-3">
                            <li>@lang('Download the sample Excel file to understand the required format')</li>
                            <li>@lang('One Excel file = One Purchase with multiple products')</li>
                            <li><strong>@lang('Required Fields:')</strong> @lang('Purchase Date, Supplier, Warehouse, Status, and at least one product')</li>
                            <li>@lang('Supplier, Warehouse, Products, Taxes, and Payment Types must already exist in the system')</li>
                            <li><strong>@lang('Transaction Rule:')</strong> @lang('If ANY validation fails, the ENTIRE import will be rejected (no partial imports)')</li>
                            <li>@lang('All pricing calculations will follow the same logic as manual purchase creation')</li>
                        </ul>
                    </div>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <a href="{{ route('user.purchase.import.sample') }}" class="btn btn-outline--success btn-sm">
                            <i class="las la-file-excel me-1"></i> @lang('Download Sample Excel File')
                        </a>
                        <p class="text-muted fs-14 mt-2 mb-0">
                            @lang('The sample file contains all required sections: Purchase Header, Discount, Payment, and Products.')
                        </p>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn--primary btn-large import-submit-btn">
                        <i class="las la-file-import me-1"></i> @lang('Import Purchase')
                    </button>
                </div>
            </form>
        </div>
...
<script>
    "use strict";
    (function($) {
        const $importForm = $('.purchase-import-form');
        ...
        const importText = @json(__('Import Purchase'));
        ...
        $('#importPurchaseModal').on('show.bs.modal', function() {
            resetModalState();
        });
